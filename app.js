...
function render() {
  ...
  document.querySelector("#app").innerHTML = `
    <header class="top"><div class="brand"><span class="mark">⌁</span> HARNESS RANGER <em>INTERACTIVE DEMO</em></div><nav class="top-links" aria-label="Demo resources"><a href="flow.html">System flow</a><a href="docs/cross-judge-validation-and-multi-track-iteration.pdf" target="_blank">Cross-Judge paper (PDF)</a><a href="features.html">Features</a></nav><span class="mode">DEMO DATA · NO LIVE CONNECTIONS</span></header>
...
function evolutionFeatures(h) {
  return { capture: true, paired: true, calibration: true, canary: false, ...(h.evolution.features || {}) };
}

function evolutionCard(h) {
  const active = h.state === "active";
  const autoReady = h.metrics.aa === "Calibrated" && h.metrics.stage !== "Rollback hold";
  const signals = evolutionFeatures(h);
  const enabled = active && h.evolution.on;
  const signalDefs = [["capture", "Candidate capture", "Immutable artifact versions and redacted inputs"], ["paired", "Paired delta tracking", "Baseline and candidate outcomes under matched conditions"], ["calibration", "A/A calibration", "Bias gate before candidate comparison"], ["canary", "Canary observation", "Live cohort outcomes after the offline gate"]];
  return `<article class="card self-evolution"><p class="eyebrow">SELF‑EVOLUTION</p><h3>Guarded artifact improvement</h3><p class="policy-note">Self‑Evolution is artifact-level self-learning: prompt sections, tool descriptions, and skills are compared independently with causal tasks.</p><label class="master"><input type="checkbox" data-evolution="master" ${h.evolution.on ? "checked" : ""} ${active ? "" : "disabled"}/><span><strong>Self‑Evolution telemetry</strong><small>Master control · ${h.evolution.on ? "on" : "off"}</small></span></label><div class="modes" role="group" aria-label="Evolution mode">${[["observe", "Observe", !h.evolution.on], ["evaluate", "Evaluate", !h.evolution.on], ["auto-canary", "Auto‑canary", !h.evolution.on || !autoReady]].map(([mode, text, disabled]) => `<button class="${h.evolution.mode === mode ? "chosen" : ""}" data-mode="${mode}" ${disabled ? "disabled" : ""}>${text}</button>`).join("")}</div><div class="evolution-funnel"><span class="done">Regression suite</span><span>→</span><span class="done">Paired benchmark ≥50</span><span>→</span><span class="${autoReady ? "done" : "hold"}">A/A control</span><span>→</span><span class="${h.evolution.mode === "auto-canary" ? "done" : "hold"}">Live canary ≥100</span><span>→</span><span class="hold">Promotion / rollback</span></div><div class="evolution-readouts"><div><small>Tracked candidates</small><strong>${h.evolution.on ? "08" : "00"}</strong></div><div><small>Paired outcomes</small><strong>${h.metrics.benchmark}</strong></div><div><small>Canary state</small><strong>${h.evolution.mode === "auto-canary" ? "Armed" : "Offline"}</strong></div></div><div class="evolution-options"><p class="eyebrow">TELEMETRY SIGNALS</p>${signalDefs.map(([key, name, note]) => `<label class="evolution-option ${enabled ? "" : "locked"}"><input type="checkbox" data-evolution-feature="${key}" ${signals[key] ? "checked" : ""} ${enabled ? "" : "disabled"}/><span><strong>${name}</strong><small>${note}</small></span></label>`).join("")}</div><dl class="artifact"><dt>Candidate artifact</dt><dd>${h.evolution.artifact}</dd><dt>Promotion rule</dt><dd>${h.metrics.stage === "Rollback hold" ? "Blocked pending safety and benchmark recovery" : "No severe safety regression; success improves; thresholds hold"}</dd></dl></article>`;
}
...
function bind() {
  ...
  document.querySelectorAll("[data-evolution-feature]").forEach((el) => el.onchange = () => { const h = harnesses.find((item) => item.id === selected); h.evolution.features = { ...evolutionFeatures(h), [el.dataset.evolutionFeature]: el.checked }; banner = `${el.parentElement.innerText.split("\n")[0]} ${el.checked ? "enabled" : "disabled"} for ${h.name}.`; render(); });
